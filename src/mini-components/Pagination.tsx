    ...
    const restOfPagination = () => {
        ...
        if( numberOfPages<5 ){
            ...
            for(let i=2; i <= numberOfPages-1; i++){
                elements.push(<Pagination.Item  key={`${i} page`} active={currentPage===i} href={link+`&page=${i}`}>{i}</Pagination.Item>);
            }
            return elements;
        }
        else{        
            if(currentPage<3 || numberOfPages===currentPage){
                ...
                for(let i=2; i <= 5; i++){
                    ...
                    if(i!==currentPage) className = "hide-on-small-screens"

                    elements.push(<Pagination.Item className={className} key={`${i} page`} active={currentPage===i} href={link+`&page=${i}`}>{i}</Pagination.Item>);
                }
            ...
            for(let i=currentPage; i <= currentPage+3; i++){
                ...
                elements.push(<Pagination.Item key={`${i} page`} className={className} active={currentPage===i} href={link+`&page=${i}`}>{i}</Pagination.Item>);
            }
