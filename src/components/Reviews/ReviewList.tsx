...
const ReviewList = ({reviews}: { reviews: movieReview[]}): JSX.Element => {
    ...
    function formatText (e: React.MouseEvent<HTMLInputElement, MouseEvent>, tag: String) { 

            let selectedText: string = window.getSelection().toString()
            let textarea = document.getElementById("a") as any
            let start = textarea.selectionStart;
            let finish = textarea.selectionEnd;
            let allText = textarea.value;
            let newTextForSelected: string = "[" + tag + "]" + selectedText + "[/" + tag + "]"; 
            let newText = 
                allText.substring(0, start) + newTextForSelected + allText.substring(finish, allText.length);
